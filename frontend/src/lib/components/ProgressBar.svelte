...
{#if segments}
  <div class="bar-segmented" style="height: {height}px">
    {#each segments as s, i}
      <div class="segment" style="width: {s.pct}%; background: {s.color || c}; border-right: {i < segments.length - 1 ? '1px solid var(--bg)' : 'none'}" />
    {/each}
  </div>
{:else}
  <div class="bar-container" style="height: {height}px">
    <div class="bar-fill" style="width: {value}%; background: {c}; border-radius: {height / 2}px" />
  </div>
{/if}
